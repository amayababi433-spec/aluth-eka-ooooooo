...
// ===========================================
//  ENGINE: PERFORMANCE & SECURITY
// ===========================================
const locks = new Set();
const cooldown = new Map();
const CD_MS = 3000; // තත්පර 3ක විරාමයක්
...
// ආරක්ෂක පියවර: අනවශ්‍ය කේත බ්ලොක් කිරීම
function isSafeFilter(q) {
    return !/[;&|`$<>]/.test(q);
}

// FFmpeg ධාවනය කිරීම (Spawn Method)
...
// Smart Send: ෆයිල් එකේ සයිස් එක අනුව යවන විදිහ තීරණය කිරීම
async function sendSmartMedia(conn, from, filePath, type, mek, caption = "") {
    const stats = fs.statSync(filePath);
    const fileSizeMB = stats.size / (1024 * 1024);

    // 16MB ට වැඩි නම් Document විදිහට යවනවා (Quality බහින්නේ නෑ + Limit ප්‍රශ්න නෑ)
    if (fileSizeMB > 16) {
...
cmd({
    ...
    try {
        ...
        if (isVideo) {
            ...
            // Video filter + Copy Audio (Fastest) + Ultrafast Preset
